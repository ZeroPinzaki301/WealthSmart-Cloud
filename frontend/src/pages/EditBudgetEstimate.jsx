...
import { TbTrash, TbPlus, TbArrowLeft, TbLogin, TbUserPlus, TbFileAnalyticsFilled } from "react-icons/tb";
import { UseCurrency } from "../hooks/UseCurrency";
import WealthSmartBG from '../assets/WealthSmartBG.png';
import { useAuth } from "../context/AuthContext";

export default function EditBudgetEstimate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`/budget-estimations/${id}`, payload);
      
      if (response.data.success) {
        navigate(`/budget-estimates/${id}`);
      } else {
        setError(response.data.error || "Failed to update budget estimation");
      }
    } catch (err) {
      console.error("Error updating budget estimation:", err);
      if (err.response?.status === 401) {
        setShowAuthModal(true);
      } else {
        setError(err.response?.data?.error || "Failed to update budget estimation");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-secondary-darkest border border-border rounded-xl max-w-md w-full p-6 text-center">
        ...
        <p className="text-text-muted mb-6 text-sm">
          You need to be logged in to edit this budget estimation.
        </p>
  ...
  if (authLoading || fetching) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-text-muted">{fetching ? "Loading estimation..." : "Loading..."}</div>
      </div>
    );
  }
      ...
      <div className="relative md:top-4 -top-[4.75em] backdrop-blur-[3px] z-10 max-w-[95%] mx-auto bg-secondary-darkest/90 border rounded-lg p-3 sm:p-6 mb-20 sm:mb-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate(`/budget-estimates/${id}`)}
              className="p-1.5 rounded-lg hover:bg-background-subtle transition-colors cursor-pointer"
            >
              <TbArrowLeft className="text-xl sm:text-2xl text-primary" />
            </button>
            <h1 className="text-lg sm:text-2xl md:text-3xl font-bold text-primary">Edit Budget Estimation</h1>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-background rounded-lg border border-border p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
              ...
              <button
                type="button"
                onClick={addExpense}
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-3 py-2 bg-primary text-white rounded-lg hover:bg-primary-darkest transition-colors cursor-pointer text-sm"
                disabled={loading}
              >
              ...
              {formData.expenses.map((expense, index) => (
                <div key={index} className="flex flex-col sm:flex-row gap-3 items-start p-3 bg-background-subtle rounded-lg border border-border">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto mt-0 sm:mt-6 px-3 py-2 text-secondary hover:bg-red-500/10 rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-1"
                    disabled={loading}
                  >
          ...
          {breakdown.periodBreakdown.length > 0 && (
            <div className="bg-background-subtle rounded-lg border border-border p-4 sm:p-6">
              <h2 className="text-base sm:text-xl font-semibold text-primary mb-4">Period-wise Budget Breakdown (Preview)</h2>
              
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 pb-6 border-b border-border">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-end">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? "Updating..." : "Update Estimation"}
            </button>
          </div>
        </form>
      </div>
